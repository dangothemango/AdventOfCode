...
def main():
    ...
    for i in range(len(lines)):
        maxHeat = max(getHeat(((i,0),'east'),lines), maxHeat)
        maxHeat = max(getHeat(((i,len(lines[i])-1),'west'),lines), maxHeat)
    for i in range(len(lines[0])):
        maxHeat = max(getHeat(((0,i),'south'),lines), maxHeat)
        maxHeat = max(getHeat(((len(lines)-1,i),'north'),lines), maxHeat)
    return maxHeat
